...
Set these on the Worker `skvallerbyttan`:

- `SKVALLERBYTTAN_WEBHOOK_SECRET` — Secret; same value as the Avkroken organization webhook secret.
- `SKVALLERBYTTAN_APP_ID` — the Gamnacken GitHub App ID.
- `SKVALLERBYTTAN_APP_PRIVATE_KEY` — Secret; the app private key in unencrypted PKCS#8 PEM format.
...
Store the complete contents of `github-app-key-pkcs8.pem` as `SKVALLERBYTTAN_APP_PRIVATE_KEY`. Never commit either private-key file.
